...
<?php $__env->startSection('breadcrumb', 'WhatsApp Automation'); ?>

<?php $__env->startSection('content'); ?>
<div class="p-6 space-y-6" x-data="whatsappDashboard()">
    <?php if(session('success')): ?>
        <div class="rounded-xl bg-green-50 text-green-800 px-4 py-3 text-sm border border-green-100"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if(session('error')): ?>
        <div class="rounded-xl bg-red-50 text-red-800 px-4 py-3 text-sm border border-red-100"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    <?php if(!empty($pageLoadError)): ?>
        <div class="rounded-xl bg-red-50 text-red-900 px-4 py-3 text-sm border border-red-200">
            <?php echo e($pageLoadError); ?>

        </div>
    <?php endif; ?>
    <?php if(isset($hasWaMessages) && ! $hasWaMessages): ?>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900"><?php echo e($stats['sent_today'] ?? 0); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900"><?php echo e($stats['received_today'] ?? 0); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-amber-600"><?php echo e($stats['pending_replies'] ?? 0); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-purple-600"><?php echo e($stats['reminders_scheduled'] ?? 0); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-teal-600"><?php echo e($stats['templates_count'] ?? 0); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <form method="POST" action="<?php echo e(route('whatsapp.teleconsult')); ?>" class="flex flex-wrap gap-3 items-end">
            <?php echo csrf_field(); ?>
            <div class="flex-1 min-w-[200px]">
                ...
                <select name="appointment_id" required class="w-full rounded-lg border border-gray-300 text-sm px-3 py-2">
                    <?php $__currentLoopData = $upcomingAppointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ap): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($ap->id); ?>"><?php echo e($ap->patient?->name ?? 'Patient'); ?> — <?php echo e(optional($ap->scheduled_at)->format('d M, h:i A') ?? '—'); ?></option>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div x-show="activeTab === 'messages'" class="p-5">
            ...
            <div class="divide-y divide-gray-200 max-h-[500px] overflow-y-auto">
                ...
                <div class="py-4 hover:bg-gray-50 transition-colors <?php echo e($message->status === 'unread' ? 'bg-amber-50' : ''); ?>">
                    <div class="flex items-start gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style="background: linear-gradient(135deg, #25D366, #128C7E);">
                            <?php echo e(strtoupper(substr($message->patient?->name ?? 'P', 0, 1))); ?>

                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                <p class="font-semibold text-gray-900"><?php echo e($message->patient?->name ?? 'Unknown'); ?></p>
                                <span class="text-xs text-gray-400"><?php echo e(optional($message->created_at)->format('d M h:i A') ?? '—'); ?></span>
                            </div>
                            <p class="text-sm text-gray-600 mt-0.5"><?php echo e($message->content); ?></p>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <input type="checkbox" value="<?php echo e($appointment->id); ?>" x-model="selectedAppointments" class="rounded border-gray-300 text-green-600 focus:ring-green-500">
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-600 text-xs font-bold">
                                        <?php echo e(strtoupper(substr($appointment->patient?->name ?? 'P', 0, 1))); ?>

                                    </div>
                                    <span class="font-medium text-gray-900"><?php echo e($appointment->patient?->name ?? 'N/A'); ?></span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($appointment->patient?->phone ?? 'N/A'); ?></td>
                            <td class="px-4 py-3">
                                <div class="text-sm font-medium text-gray-900"><?php echo e(optional($appointment->scheduled_at)->format('d M Y') ?? '—'); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e(optional($appointment->scheduled_at)->format('h:i A') ?? ''); ?></div>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600">Dr. <?php echo e($appointment->doctor?->name ?? 'N/A'); ?></td>
                            <td class="px-4 py-3 text-center">
                                <button @click="sendSingleReminder(<?php echo e($appointment->id); ?>)" class="text-green-600 hover:text-green-800" title="Send Reminder">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <div class="flex items-start justify-between">
                        <div>
                            <h4 class="font-semibold text-gray-900"><?php echo e($template->name); ?></h4>
                            <span class="inline-flex px-2 py-0.5 text-xs font-medium rounded-full mt-1 
                                <?php echo e($template->type === 'appointment_reminder' ? 'bg-blue-100 text-blue-700' : ''); ?>

                                <?php echo e($template->type === 'prescription' ? 'bg-green-100 text-green-700' : ''); ?>

                                <?php echo e($template->type === 'follow_up' ? 'bg-amber-100 text-amber-700' : ''); ?>

                                <?php echo e($template->type === 'birthday' ? 'bg-pink-100 text-pink-700' : ''); ?>

                                <?php echo e($template->type === 'custom' ? 'bg-gray-100 text-gray-700' : ''); ?>
                        ...
                        <button @click="deleteTemplate(<?php echo e($template->id); ?>)" class="text-gray-400 hover:text-red-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                        </button>
                    </div>
                    <p class="text-sm text-gray-600 mt-2 whitespace-pre-wrap"><?php echo e(Str::limit($template->content, 150)); ?></p>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
    ...
    <div x-show="showSendModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" x-cloak>
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4" @click.outside="showSendModal = false">
            ...
            <form action="<?php echo e(route('whatsapp.send')); ?>" method="POST" class="p-6 space-y-4">
                <?php echo csrf_field(); ?>
                <div>
                    ...
                    <select name="patient_id" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm bg-white">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = ($patients ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <option value="<?php echo e($p->id); ?>" <?php echo e((string) old('patient_id') === (string) $p->id ? 'selected' : ''); ?>>
                            <?php echo e($p->name); ?> — <?php echo e($p->phone); ?>
...
function whatsappDashboard() {
    return {
        ...
        automationSettings: <?php echo e(\Illuminate\Support\Js::from($waAutomationForJs)); ?>,
        newTemplate: {
            name: '',
        ...
        toggleAllAppointments(event) {
            if (event.target.checked) {
                this.selectedAppointments = <?php echo e(\Illuminate\Support\Js::from($waAppointmentIdsForJs)); ?>;
            } else {
        ...
        async saveAutomationSetting(type) {
            ...
            try {
                const response = await fetch(<?php echo e(\Illuminate\Support\Js::from($waRemindersScheduleUrl)); ?>, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/whatsapp/index.blade.php ENDPATH**/ ?>