<?php $__env->startSection('title', 'IPD — ' . ($admission->patient->name ?? 'Patient')); ?>
<?php $__env->startSection('breadcrumb', 'IPD · ' . ($admission->admission_number ?? 'Admission')); ?>
...
?>
<div x-data="ipdShow(<?php echo e(\Illuminate\Support\Js::from($ipdInitialTab)); ?>)" class="p-4 sm:p-5 lg:p-7 space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
            <h1 class="text-xl font-bold text-gray-900 font-display"><?php echo e($admission->patient->name ?? 'Patient'); ?></h1>
            <p class="text-sm text-gray-500">Admission #<?php echo e($admission->admission_number); ?></p>
        </div>
        ...
        <a href="<?php echo e(route('ipd.print-prescription', $admission)); ?>" target="_blank"
           class="inline-flex items-center gap-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-gradient-to-r from-slate-50 to-blue-50/80 border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            <?php if(\Illuminate\Support\Facades\Route::has('billing.create')): ?>
                <a href="<?php echo e(route('billing.create', ['patient_id' => $ipdPid, 'admission_id' => $ipdAdmId])); ?>"
            ...
            <?php if(\Illuminate\Support\Facades\Route::has('billing.index')): ?>
                <a href="<?php echo e(route('billing.index', ['patient_id' => $ipdPid, 'admission_id' => $ipdAdmId])); ?>"
            ...
            <?php endif; ?>
            <?php if(\Illuminate\Support\Facades\Route::has('laboratory.orders')): ?>
                <a href="<?php echo e(route('laboratory.orders', ['patient_id' => $ipdPid])); ?>"
            ...
            <?php endif; ?>
            <?php if(\Illuminate\Support\Facades\Route::has('lab.index')): ?>
            ...
            <?php endif; ?>
            <?php if(\Illuminate\Support\Facades\Route::has('pharmacy.dispense.form')): ?>
                <a href="<?php echo e(route('pharmacy.dispense.form', ['patient_id' => $ipdPid, 'admission_id' => $ipdAdmId])); ?>"
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5">
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-5">
            <div>
                ...
                <p class="text-sm font-semibold text-gray-900"><?php echo e($admission->patient->name ?? '—'); ?></p>
                <p class="text-xs text-gray-500"><?php echo e($admission->patient->phone ?? ''); ?></p>
            </div>
            <div>
                <p class="text-xs font-medium text-gray-400 mb-1">Age / Gender</p>
                <p class="text-sm font-semibold text-gray-900">
                    <?php echo e($admission->patient->age_years ?? '—'); ?> yr
                    <?php if($admission->patient->sex ?? null): ?> · <?php echo e(ucfirst($admission->patient->sex)); ?> <?php endif; ?>
                </p>
            </div>
            <div>
                <p class="text-xs font-medium text-gray-400 mb-1">Ward / Bed</p>
                <p class="text-sm font-semibold text-gray-900"><?php echo e($admission->ward->name ?? '—'); ?></p>
                <p class="text-xs text-gray-500">Bed <?php echo e($admission->bed->bed_number ?? '—'); ?></p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="text-sm font-semibold text-gray-900"><?php echo e($admission->primaryDoctor->name ?? '—'); ?></p>
            </div>
            <div>
                <p class="text-xs font-medium text-gray-400 mb-1">Diagnosis</p>
                <p class="text-sm font-semibold text-gray-900 truncate" title="<?php echo e($admission->diagnosis_at_admission); ?>">
                ...
                <span class="inline-flex items-center mt-1 px-2 py-0.5 rounded-full text-xs font-semibold"
                      style="background:<?php echo e($s['bg']); ?>;color:<?php echo e($s['color']); ?>;">
                    <?php echo e($s['label']); ?>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="border-b border-gray-100 flex overflow-x-auto">
            <?php $__currentLoopData = ['notes' => 'Progress Notes', 'vitals' => 'Vitals Chart', 'medications' => 'Medications & MAR', 'handover' => 'Handover', 'care' => 'Care Plans', 'discharge' => 'Discharge Summary']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tabKey => $tabLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button @click="activeTab = '<?php echo e($tabKey); ?>'"
                ...
                style="border-color: activeTab === '<?php echo e($tabKey); ?>' ? '#1447E6' : 'transparent';">
                <?php echo e($tabLabel); ?>
        ...
        <div x-show="activeTab === 'notes'" class="p-5 space-y-4">
            ...
            <?php $__empty_1 = true; $__currentLoopData = $progressNotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $note): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php $soap = $note->soapPayload(); ?>
            <div class="border border-gray-100 rounded-xl p-4 space-y-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700"><?php echo e(ucfirst($soap['note_type'] ?? $note->note_type ?? 'note')); ?></span>
                        <span class="text-sm font-semibold text-gray-900"><?php echo e($note->author->name ?? 'Staff'); ?></span>
                    </div>
                    <span class="text-xs text-gray-400">
                        <?php if($note->note_at ?? null): ?>
                            <?php echo e(\Carbon\Carbon::parse($note->note_at)->format('d M Y H:i')); ?>

                        <?php elseif($note->note_date): ?>
                            <?php echo e(\Carbon\Carbon::parse($note->note_date)->format('d M Y')); ?>

                            <?php echo e($note->note_time ?? ''); ?>

                        <?php endif; ?>
                    </span>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <?php if(!empty($soap['subjective'] ?? $note->subjective)): ?>
                    <div><p class="text-xs font-semibold text-gray-400 mb-0.5">Subjective</p><p class="text-gray-700"><?php echo e($soap['subjective'] ?? $note->subjective); ?></p></div>
                    <?php endif; ?>
                    <?php if(!empty($soap['objective'] ?? $note->objective)): ?>
                    <div><p class="text-xs font-semibold text-gray-400 mb-0.5">Objective</p><p class="text-gray-700"><?php echo e($soap['objective'] ?? $note->objective); ?></p></div>
                    <?php endif; ?>
                    <?php if(!empty($soap['assessment'] ?? $note->assessment)): ?>
                    <div><p class="text-xs font-semibold text-gray-400 mb-0.5">Assessment</p><p class="text-gray-700"><?php echo e($soap['assessment'] ?? $note->assessment); ?></p></div>
                    <?php endif; ?>
                    <?php if(!empty($soap['plan'] ?? $note->plan)): ?>
                    <div><p class="text-xs font-semibold text-gray-400 mb-0.5">Plan</p><p class="text-gray-700"><?php echo e($soap['plan'] ?? $note->plan); ?></p></div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-3 py-2 text-xs text-gray-500"><?php echo e(\Carbon\Carbon::parse($vital->recorded_at)->format('d M H:i')); ?></td>
                            <td class="px-3 py-2"><?php echo e($vital->temperature_display !== null ? number_format((float) $vital->temperature_display, 1).'°C' : '—'); ?></td>
                            <td class="px-3 py-2"><?php echo e($vital->pulse ? $vital->pulse.' bpm' : '—'); ?></td>
                            <td class="px-3 py-2">
                                <?php if($vital->bp_systolic && $vital->bp_diastolic): ?>
                                    <?php echo e($vital->bp_systolic); ?>/<?php echo e($vital->bp_diastolic); ?>
                            ...
                            <td class="px-3 py-2">
                                <?php if($vital->spo2): ?>
                                    <span class="<?php echo e($vital->spo2 < 95 ? 'text-red-600 font-semibold' : ''); ?>"><?php echo e($vital->spo2); ?>%</span>
                                <?php else: ?> —
                                <?php endif; ?>
                            </td>
                            <td class="px-3 py-2"><?php echo e($vital->respiratory_rate_display !== null ? $vital->respiratory_rate_display.'/min' : '—'); ?></td>
                            <td class="px-3 py-2"><?php echo e($vital->pain_score !== null ? $vital->pain_score.'/10' : '—'); ?></td>
                            <td class="px-3 py-2 text-xs text-gray-500"><?php echo e($vital->recordedBy->name ?? '—'); ?></td>
                        </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr>
        ...
        <div x-show="activeTab === 'medications'" class="p-5 space-y-6">
            <?php if($admission->status === 'admitted'): ?>
            <form method="POST" action="<?php echo e(route('ipd.medication-orders.store', $admission)); ?>" class="border border-gray-100 rounded-xl p-4 space-y-3 bg-slate-50/50">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="drug_name" required value="<?php echo e(old('drug_name')); ?>"
                    ...
                    <div>
                        ...
                        <select name="route" required class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white">
                            <?php $__currentLoopData = ['oral' => 'Oral', 'iv' => 'IV', 'im' => 'IM', 'sc' => 'SC', 'topical' => 'Topical', 'sublingual' => 'Sublingual', 'inhalation' => 'Inhalation', 'rectal' => 'Rectal']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rv => $rl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($rv); ?>" <?php if(old('route') === $rv): echo 'selected'; endif; ?>><?php echo e($rl); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Dosage</label>
                        <input type="text" name="dosage" required value="<?php echo e(old('dosage')); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="500 mg">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Frequency</label>
                        <input type="text" name="frequency" required value="<?php echo e(old('frequency')); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="TDS / OD">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Start</label>
                        <input type="date" name="start_date" required value="<?php echo e(old('start_date', date('Y-m-d'))); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">End (optional)</label>
                        <input type="date" name="end_date" value="<?php echo e(old('end_date')); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Instructions</label>
                        <input type="text" name="instructions" value="<?php echo e(old('instructions')); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="After food, etc.">
                    </div>
                    <div class="flex items-center gap-2 pt-6">
                        <input type="checkbox" name="is_sos" id="is_sos" value="1" class="rounded border-gray-300" <?php if(old('is_sos')): echo 'checked'; endif; ?>>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $medicationOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $med): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="border border-gray-100 rounded-xl overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-start gap-3 p-4 bg-white">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-gray-900"><?php echo e($med->drug_name ?? '—'); ?></p>
                        <p class="text-xs text-gray-500 mt-0.5"><?php echo e($med->dosage ?? ''); ?> · <?php echo e($med->route ?? ''); ?> · <?php echo e($med->frequency ?? ''); ?></p>
                        <p class="text-xs text-gray-400 mt-1">By <?php echo e($med->prescribedBy->name ?? '—'); ?> · <?php echo e(ucfirst($med->status ?? 'active')); ?></p>
                    </div>
                </div>
                <?php if($admission->status === 'admitted' && ($med->status ?? '') === 'active'): ?>
                <div class="px-4 pb-4 border-t border-gray-50 bg-gray-50/80">
                    ...
                    <form method="POST" action="<?php echo e(route('ipd.mar.store', $admission)); ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 items-end">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="order_id" value="<?php echo e($med->id); ?>">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="administered_at" required value="<?php echo e(now()->format('Y-m-d\TH:i')); ?>" class="w-full px-2 py-1.5 border rounded text-sm">
                        </div>
                        <div>
                        ...
                        <div class="flex items-center gap-2 pb-0.5">
                            <input type="checkbox" name="not_administered" value="1" id="na_<?php echo e($med->id); ?>" class="rounded">
                            <label for="na_<?php echo e($med->id); ?>" class="text-xs text-gray-600">Not given</label>
                        </div>
                ...
                <?php endif; ?>
                <?php if($med->administrations && $med->administrations->count()): ?>
                <div class="px-4 py-3 bg-white border-t border-gray-100">
                    <p class="text-xs font-semibold text-gray-500 mb-2">Recent MAR</p>
                    <ul class="space-y-1 text-xs text-gray-700">
                        <?php $__currentLoopData = $med->administrations->sortByDesc('administered_at')->take(8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $adm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li class="flex flex-wrap gap-x-3 gap-y-0.5">
                            <span><?php echo e($adm->administered_at ? \Carbon\Carbon::parse($adm->administered_at)->format('d M H:i') : ''); ?></span>
                            <span><?php echo e($adm->dose_given); ?></span>
                            <?php if($adm->not_administered): ?>
                                <span class="text-amber-700">Not given<?php echo e($adm->not_administered_reason ? ': '.$adm->not_administered_reason : ''); ?></span>
                            <?php endif; ?>
                            <span class="text-gray-400"><?php echo e($adm->administeredBy->name ?? ''); ?></span>
                        </li>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </ul>
                </div>
        ...
        <div x-show="activeTab === 'handover'" class="p-5 space-y-4">
            <?php if($admission->status === 'admitted'): ?>
            <form method="POST" action="<?php echo e(route('ipd.handover.store', $admission)); ?>" class="border border-gray-100 rounded-xl p-4 space-y-3 bg-amber-50/30">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <textarea name="summary" required rows="4" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="Patient status, lines, pending tasks…"><?php echo e(old('summary')); ?></textarea>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Concerns / risks</label>
                    <textarea name="concerns" rows="2" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm"><?php echo e(old('concerns')); ?></textarea>
                </div>
            ...
            <div class="space-y-3">
                ...
                <?php $__empty_1 = true; $__currentLoopData = $handoverNotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $h): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="border border-gray-100 rounded-xl p-4 space-y-2">
                    <div class="flex flex-wrap justify-between gap-2 text-xs text-gray-500">
                        <span><?php echo e($h->created_at?->format('d M Y H:i')); ?></span>
                        <span><?php echo e($h->shift ? ucfirst($h->shift) : 'Shift'); ?> · <?php echo e($h->handedOverBy->name ?? 'Staff'); ?></span>
                    </div>
                    <p class="text-sm text-gray-800 whitespace-pre-wrap"><?php echo e($h->summary); ?></p>
                    <?php if($h->concerns): ?>
                    <p class="text-xs text-amber-900 bg-amber-50 rounded-lg p-2"><span class="font-semibold">Concerns:</span> <?php echo e($h->concerns); ?></p>
                    <?php endif; ?>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        ...
        <div x-show="activeTab === 'care'" class="p-5 space-y-4">
            <?php if($admission->status === 'admitted'): ?>
            <form method="POST" action="<?php echo e(route('ipd.care-plan.store', $admission)); ?>" class="border border-gray-100 rounded-xl p-4 space-y-3 bg-indigo-50/30">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Goal <span class="text-red-500">*</span></label>
                        <input type="text" name="goal" required value="<?php echo e(old('goal')); ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="e.g. Maintain SpO2 above 94%">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Interventions</label>
                    <textarea name="interventions" rows="3" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm"><?php echo e(old('interventions')); ?></textarea>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Outcome review</label>
                    <textarea name="outcome_review" rows="2" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm"><?php echo e(old('outcome_review')); ?></textarea>
                </div>
            ...
            <?php endif; ?>
            <div class="space-y-3">
                <?php $__empty_1 = true; $__currentLoopData = $carePlans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="border border-gray-100 rounded-xl p-4">
                    <p class="text-sm font-semibold text-gray-900"><?php echo e($cp->goal); ?></p>
                    <?php if($cp->interventions): ?>
                    <p class="text-sm text-gray-700 mt-2 whitespace-pre-wrap"><?php echo e($cp->interventions); ?></p>
                    <?php endif; ?>
                    <?php if($cp->outcome_review): ?>
                    <p class="text-xs text-gray-500 mt-2"><span class="font-semibold">Review:</span> <?php echo e($cp->outcome_review); ?></p>
                    <?php endif; ?>
                    <p class="text-xs text-gray-400 mt-2"><?php echo e($cp->updated_at?->format('d M Y H:i')); ?></p>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-semibold text-gray-900"><?php echo e($admission->discharge_date ? \Carbon\Carbon::parse($admission->discharge_date)->format('d M Y, H:i') : '—'); ?></p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-400 mb-1">Discharge Type</p>
                        <p class="font-semibold text-gray-900"><?php echo e(ucfirst($admission->discharge_type ?? '—')); ?></p>
                    </div>
                    <div class="sm:col-span-2">
                        <p class="text-xs font-medium text-gray-400 mb-1">Final Diagnosis</p>
                        <p class="text-gray-700"><?php echo e($admission->final_diagnosis ?? '—'); ?></p>
                    </div>
                    <div class="sm:col-span-2">
                        <p class="text-xs font-medium text-gray-400 mb-1">Discharge Notes</p>
                        <p class="text-gray-700"><?php echo e($admission->discharge_notes ?? '—'); ?></p>
                    </div>
                </div>
            </div>
            <?php else: ?>
            <form method="POST" action="<?php echo e(route('ipd.discharge', $admission)); ?>" class="space-y-4 max-w-xl">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <input type="text" name="final_diagnosis" required
                        value="<?php echo e(old('final_diagnosis', $admission->diagnosis_at_admission)); ?>"
                ...
                <div>
                    ...
                    <textarea name="discharge_notes" rows="4"
                        ...
                        class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"><?php echo e(old('discharge_notes')); ?></textarea>
                </div>
    ...
    <div x-show="showNoteModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <form method="POST" action="<?php echo e(route('ipd.progress-notes.store', $admission)); ?>" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="note_date" required value="<?php echo e(date('Y-m-d')); ?>"
                                class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Time</label>
                        <input type="time" name="note_time" required value="<?php echo e(date('H:i')); ?>"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <?php $__currentLoopData = ['subjective' => 'Subjective (S)', 'objective' => 'Objective (O)', 'assessment' => 'Assessment (A)', 'plan' => 'Plan (P)']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1"><?php echo e($label); ?></label>
                        <textarea name="<?php echo e($field); ?>" required rows="2"
    ...
    <div x-show="showDischargeModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full">
                ...
                <form method="POST" action="<?php echo e(route('ipd.discharge', $admission)); ?>" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Discharge Type <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Final Diagnosis <span class="text-red-500">*</span></label>
                        <input type="text" name="final_diagnosis" required
                            value="<?php echo e($admission->diagnosis_at_admission); ?>"
                            class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Discharge Notes</label>
...
function ipdShow(initialTab) {
    ...
    return {
        ...
        async saveVitals() {
            ...
            try {
                const res = await fetch('/ipd/<?php echo e($admission->id); ?>/vitals', {
                    method: 'POST',
                    headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/ipd/show.blade.php ENDPATH**/ ?>