...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'ClinicOS'); ?> — ClinicOS</title>
...
<div class="fixed top-0 left-0 right-0 z-[9999] bg-indigo-600 text-white px-3 sm:px-4 py-2">
    <div class="max-w-7xl mx-auto flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start sm:items-center gap-3 min-w-0">
            ...
            <span class="text-xs sm:text-sm font-medium break-words">You are viewing as <strong><?php echo e(auth()->user()->name); ?></strong> (<?php echo e(auth()->user()->clinic?->name ?? 'N/A'); ?>)</span>
        </div>
        <a href="<?php echo e(route('admin.stop-impersonating')); ?>" class="inline-flex shrink-0 items-center justify-center gap-2 px-4 py-1.5 bg-white text-indigo-600 text-sm font-semibold rounded-lg hover:bg-indigo-50 transition-colors w-full sm:w-auto">
...
>
    ...
    <div class="flex-1">
        <p class="text-sm font-600 text-gray-900"><?php echo e(session('success')); ?></p>
    </div>
...
>
    ...
    <div class="flex-1">
        <p class="text-sm font-semibold text-gray-900"><?php echo e(session('error')); ?></p>
    </div>
    <button @click="show = false" class="text-gray-400 hover:text-gray-600">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
    </button>
    ...
    >
        ...
        <div class="px-3 pt-4 pb-2 border-b border-white/[0.06]" x-data="{ open: false }">
            ...
            >
                ...
                <div class="flex-shrink-0 rounded-lg bg-white/[0.08] ring-1 ring-white/10 overflow-hidden flex items-center justify-center"
                    ...
                    <img src="<?php echo e(asset('images/clinicos-logo.png')); ?>"
                         alt="ClinicOS"
                         width="200"
                         height="48"
                         loading="eager"
                ...
                <div x-show="sidebarOpen" class="flex-1 text-left min-w-0">
                    <div class="text-white font-semibold text-sm truncate leading-tight">
                        <?php echo e(auth()->user()?->clinic?->name ?? 'ClinicOS'); ?>
            ...
            >
                ...
                ?>
                <?php $__currentLoopData = $userClinics; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $clinic): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="#" class="flex items-center gap-2.5 px-3 py-2.5 hover:bg-white/[0.05] transition-colors">
                    <div class="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                        ...
                        <?php echo e(substr($clinic->name, 0, 1)); ?>

                    </div>
                    <div>
                        <div class="text-white text-xs font-semibold"><?php echo e($clinic->name); ?></div>
                        <div class="text-gray-500 text-xs"><?php echo e(is_array($clinic->specialties ?? null) && count($clinic->specialties) ? ($clinic->specialties[0] ?? '') : ''); ?></div>
                    </div>
                </a>
        ...
        <nav class="flex-1 px-3 py-2 overflow-y-auto">
            ...
            <div class="mb-4">
                ...
                <div class="space-y-0.5">
                    ...
                    ?>
                    <a href="<?php echo e(route_exists($item['route']) ? route($item['route']) : '#'); ?>"
                    ...
                    >
                        ...
                        <?php endif; ?>
                        <svg class="flex-shrink-0 w-[18px] h-[18px] <?php echo e($isActive ? 'text-blue-400' : 'text-gray-500 group-hover:text-gray-400'); ?>"
                        ...
                        <span x-show="sidebarOpen" class="px-1.5 py-0.5 text-[10px] font-bold rounded-full <?php echo e(($item['badgeColor'] ?? 'blue') === 'red' ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'); ?>">
        ...
        <div class="border-t border-white/[0.06] p-3" x-data="{ menuOpen: false }">
            ...
            >
                ...
                <div x-show="sidebarOpen" class="flex-1 text-left min-w-0">
                    <div class="text-white text-xs font-semibold truncate"><?php echo e(auth()->user()?->name ?? 'Doctor'); ?></div>
                    <div class="text-gray-500 text-xs truncate"><?php echo e(auth()->user()?->role ?? 'Physician'); ?></div>
                </div>
            ...
            <div
                ...
                style="background-color: #161b27;"
            >
                <a href="<?php echo e(route_exists('profile') ? route('profile') : '#'); ?>" class="flex items-center gap-2.5 px-3 py-2.5 text-gray-400 hover:text-white hover:bg-white/[0.05] transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    My Profile
                </a>
                <a href="<?php echo e(route_exists('settings') ? route('settings') : '#'); ?>" class="flex items-center gap-2.5 px-3 py-2.5 text-gray-400 hover:text-white hover:bg-white/[0.05] transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Settings
                </a>
                <div class="border-t border-white/[0.06]">
                    <form method="POST" action="<?php echo e(route_exists('logout') ? route('logout') : '/logout'); ?>">
                        <?php echo csrf_field(); ?>
    ...
    <div class="flex-1 flex flex-col min-w-0 w-full overflow-hidden">
        ...
        <header class="flex-shrink-0 bg-white border-b border-gray-200 px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-4">
            ...
            <div class="flex items-center gap-2 text-sm min-w-0 flex-1">
                ...
                <a href="<?php echo e(\Illuminate\Support\Facades\Route::has($headerHomeRoute) ? route($headerHomeRoute) : '#'); ?>" class="text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0 inline-flex" aria-label="Home">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <span class="text-gray-600 font-medium truncate"><?php echo $__env->yieldContent('breadcrumb'); ?></span>
                <?php endif; ?>
            </div>
            ...
            <div class="flex items-center gap-1 sm:gap-3 flex-shrink-0">
                ...
                <div class="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 border border-blue-100 rounded-full max-w-[10rem] lg:max-w-none">
                    ...
                    <span class="text-blue-700 text-xs font-semibold"><?php echo e(auth()->user()?->clinic?->name ?? 'ClinicOS'); ?></span>
                </div>
                ...
                <button type="button" class="relative p-2 rounded-xl hover:bg-gray-100 transition-colors text-gray-500 hover:text-gray-700" aria-label="Notifications">
                    ...
                    <?php if(($headerNotificationCount ?? 0) > 0): ?>
                    <span class="absolute top-1 right-1 min-w-[1rem] h-4 px-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                        <?php echo e($headerNotificationCount > 9 ? '9+' : $headerNotificationCount); ?>
        ...
        <footer class="flex-shrink-0 bg-white border-t border-gray-100 px-3 sm:px-6 py-2">
            <p class="text-xs text-gray-400 text-center">
                ClinicOS v2.0 &middot; PHP <?php echo e(PHP_VERSION); ?> &middot; Laravel <?php echo e(app()->version()); ?>
...
<?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/layouts/app.blade.php ENDPATH**/ ?>