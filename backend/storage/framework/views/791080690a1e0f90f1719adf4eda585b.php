...
<?php $__env->startSection('breadcrumb', 'Prescriptions'); ?>

<?php $__env->startSection('content'); ?>
<div class="p-6 space-y-6">
    <?php if((isset($prescriptionItemsTableReady) && !$prescriptionItemsTableReady) || (isset($visitPrescriptionColumnsReady) && !$visitPrescriptionColumnsReady)): ?>
    <div class="rounded-xl border border-amber-200 bg-amber-50 text-amber-900 px-4 py-3 text-sm space-y-1">
        <?php if(isset($prescriptionItemsTableReady) && !$prescriptionItemsTableReady): ?>
        ...
        <?php endif; ?>
        <?php if(isset($visitPrescriptionColumnsReady) && !$visitPrescriptionColumnsReady): ?>
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="<?php echo e(route('emr.index')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1"><?php echo e($stats['today'] ?? 0); ?></p>
            <p class="text-xs text-gray-400 mt-1">Prescriptions written</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <p class="text-xs text-gray-500 font-medium">This Week</p>
            <p class="text-2xl font-bold text-gray-900 mt-1"><?php echo e($stats['week'] ?? 0); ?></p>
            <p class="text-xs text-gray-400 mt-1">Prescriptions</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <p class="text-xs text-gray-500 font-medium">Sent via WhatsApp</p>
            <p class="text-2xl font-bold text-green-600 mt-1"><?php echo e($stats['sent_via_whatsapp'] ?? 0); ?></p>
            <p class="text-xs text-gray-400 mt-1">This month</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <p class="text-xs text-gray-500 font-medium">Templates</p>
            <p class="text-2xl font-bold text-purple-600 mt-1"><?php echo e($stats['templates'] ?? 0); ?></p>
            <p class="text-xs text-gray-400 mt-1">Saved</p>
        </div>
    </div>

    
    <?php if(isset($topDrugs) && $topDrugs->count() > 0): ?>
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">Top Prescribed Drugs (This Month)</h3>
        <div class="flex flex-wrap gap-2">
            <?php $__currentLoopData = $topDrugs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $drug): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-teal-50 border border-teal-200 rounded-full">
                <span class="text-sm font-medium text-teal-700"><?php echo e($drug->drug_name); ?></span>
                <span class="text-xs font-bold text-teal-500 bg-teal-100 px-1.5 py-0.5 rounded-full"><?php echo e($drug->count); ?></span>
            </div>
        ...
        <form method="GET" class="flex flex-wrap items-center gap-4">
            ...
            <div>
                ...
                <select name="sent" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="yes" <?php echo e(request('sent') === 'yes' ? 'selected' : ''); ?>>Sent</option>
                    <option value="no" <?php echo e(request('sent') === 'no' ? 'selected' : ''); ?>>Not Sent</option>
                </select>
            </div>
            <div class="flex items-end gap-2">
                ...
                <a href="<?php echo e(route('prescriptions.index')); ?>" class="px-4 py-2 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition-colors">
                    Clear
                </a>
            </div>
        </form>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <div class="inline-flex items-center gap-2 px-3 py-2 bg-purple-50 border border-purple-200 rounded-lg cursor-pointer hover:bg-purple-100 transition-colors group">
                <span class="text-sm font-medium text-purple-700"><?php echo e($template->name); ?></span>
                <span class="text-xs text-purple-500">(<?php echo e($template->medication_count); ?> drugs)</span>
                <button onclick="deleteTemplate(<?php echo e($template->id); ?>)" class="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <?php if(isset($recentPrescriptions) && $recentPrescriptions->count() > 0): ?>
        <div class="divide-y divide-gray-100">
            <?php $__currentLoopData = $recentPrescriptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prescription): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="p-5 hover:bg-gray-50 transition-colors">
                <div class="flex items-start gap-4">
                    ...
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-white font-bold flex-shrink-0">
                        <?php echo e(strtoupper(substr($prescription->patient?->name ?? 'P', 0, 1))); ?>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between">
                            <div>
                                <h4 class="text-sm font-semibold text-gray-900"><?php echo e($prescription->patient?->name ?? 'Unknown Patient'); ?></h4>
                                <p class="text-xs text-gray-500 mt-0.5">
                                    by <?php echo e($prescription->doctor?->name ?? 'Unknown Doctor'); ?>
                            ...
                            <div class="flex items-center gap-2">
                                <?php if($prescription->whatsapp_sent_at): ?>
                        ...
                        <?php if($prescription->prescriptionItems && $prescription->prescriptionItems->count() > 0): ?>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <?php $__currentLoopData = $prescription->prescriptionItems->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 rounded-lg text-xs">
                                <span class="font-medium text-gray-700"><?php echo e($item->drug_name); ?></span>
                                <span class="text-gray-500"><?php echo e($item->dosage); ?></span>
                            </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php if($prescription->prescriptionItems->count() > 5): ?>
                            <span class="text-xs text-gray-400">+<?php echo e($prescription->prescriptionItems->count() - 5); ?> more</span>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="mt-3 flex items-center gap-3">
                            <a href="<?php echo e(route('prescriptions.pdf', $prescription)); ?>" target="_blank" class="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                Download PDF
                            </a>
                            <a href="<?php echo e(route('emr.show', ['patient' => $prescription->patient_id, 'visit' => $prescription->id])); ?>" class="text-xs text-gray-600 hover:text-gray-800 font-medium flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                View Visit
                            </a>
                            <a href="<?php echo e(route('patients.show', $prescription->patient_id)); ?>" class="text-xs text-gray-600 hover:text-gray-800 font-medium flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                </svg>
                                Patient Profile
                            </a>
                            <button onclick="sendWhatsApp(<?php echo e($prescription->id); ?>)" class="text-xs text-green-600 hover:text-green-800 font-medium flex items-center gap-1">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
                                </svg>
                                WhatsApp
        ...
        <div class="p-12 text-center">
            ...
            <a href="<?php echo e(route('emr.index')); ?>" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Go to EMR
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/prescriptions/index.blade.php ENDPATH**/ ?>