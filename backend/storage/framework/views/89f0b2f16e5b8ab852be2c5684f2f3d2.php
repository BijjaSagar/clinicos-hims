    ...
    <div class="form-body" x-show="open" x-collapse>
        <div class="form-row form-row-3">
            <div class="field-group">
                ...
                <input type="text" name="dental_proc_teeth" class="field-input" placeholder="e.g. 16, 17" value="<?php echo e($visit->getStructuredField('dental.proc_teeth') ?? ''); ?>" @input="window.triggerAutoSave && window.triggerAutoSave()">
            </div>
            <div class="field-group">
        ...
        <div class="form-row form-row-2" style="margin-top:10px">
            <div class="field-group">
                ...
                <input type="text" name="dental_material" class="field-input" placeholder="e.g. Composite A2, GIC" value="<?php echo e($visit->getStructuredField('dental.material') ?? ''); ?>" @input="window.triggerAutoSave && window.triggerAutoSave()">
            </div>
            <div class="field-group">
                <label class="field-label">Shade (if applicable)</label>
                <input type="text" name="dental_shade" class="field-input" placeholder="e.g. A2, A3" value="<?php echo e($visit->getStructuredField('dental.shade') ?? ''); ?>" @input="window.triggerAutoSave && window.triggerAutoSave()">
            </div>
        </div>
        
        <div class="field-group" style="margin-top:10px">
            <label class="field-label">Procedure Notes</label>
            <textarea name="dental_proc_notes" class="field-textarea" rows="3" placeholder="Detailed procedure notes..." @input="window.triggerAutoSave && window.triggerAutoSave()"><?php echo e($visit->getStructuredField('dental.proc_notes') ?? ''); ?></textarea>
        </div>
    </div>
</div>
...
<div class="form-section" x-data="{ open: true }">
    ...
    <div class="form-body" x-show="open" x-collapse>
        <div class="photo-grid" style="grid-template-columns:repeat(3, 1fr)">
            ...
            <?php $__currentLoopData = $visit->photos->where('condition_tag', 'xray') ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="photo-thumb" style="aspect-ratio:4/3">
                <img src="<?php echo e(route('patients.view-photo', [$patient, $photo->id])); ?>" alt="X-ray" style="width:100%;height:100%;object-fit:cover">
                <div style="position:absolute;bottom:4px;left:4px;padding:2px 6px;background:rgba(0,0,0,0.6);color:white;font-size:9px;border-radius:4px">
                    <?php echo e($photo->body_region ?? 'X-Ray'); ?>
        ...
        <div class="field-group" style="margin-top:10px">
            ...
            <textarea name="dental_xray_findings" class="field-textarea" rows="2" placeholder="Describe findings..." @input="window.triggerAutoSave && window.triggerAutoSave()"><?php echo e($visit->getStructuredField('dental.xray_findings') ?? ''); ?></textarea>
        </div>
    </div>
</div>
...
function dentalChart() {
    ...
    const existingTeeth = <?php echo json_encode($patient->dentalTeeth ?? [], 15, 512) ?>;
    const teethData = {};
...
function dentalTreatmentPlan() {
    return {
        ...
        treatments: <?php echo json_encode($visit->getStructuredField('dental.treatment_plan') ?? $defaultDentalTreatmentPlan, 15, 512) ?>,
...
function labWorkSection() {
    return {
        open: true,
        labOrders: <?php echo json_encode($visit->getStructuredField('dental.lab_orders') ?? [], 15, 512) ?>,
...
<?php $__env->stopPush(); ?>
<?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/emr/specialty/dental.blade.php ENDPATH**/ ?>