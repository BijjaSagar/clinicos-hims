...
<?php $__env->startSection('breadcrumb', 'Patients'); ?>
...
<div class="p-6 space-y-5" x-data="{ activeFilter: 'all' }">
    ...
    <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
            <div class="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-xl">
                ...
                <span class="text-sm font-bold text-gray-900"><?php echo e($stats['total'] ?? 248); ?></span>
            </div>
            <div class="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-xl">
                <span class="w-2 h-2 rounded-full bg-green-500 flex-shrink-0"></span>
                <span class="text-xs text-gray-500">New this month</span>
                <span class="text-sm font-bold text-gray-900"><?php echo e($stats['new_month'] ?? 12); ?></span>
            </div>
            <div class="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-xl">
                <span class="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0"></span>
                <span class="text-xs text-gray-500">Active cases</span>
                <span class="text-sm font-bold text-gray-900"><?php echo e($stats['active'] ?? 34); ?></span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <?php $__empty_1 = true; $__currentLoopData = $patients ?? $demoPatients; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $patient): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <a href="<?php echo e(route('patients.show', $patient['id'] ?? $patient->id)); ?>"
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="relative flex-shrink-0">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-base font-display"
                         style="background:linear-gradient(135deg,<?php echo e($patient['gradient'] ?? $patient->gradient ?? '#1447E6,#0891B2'); ?>)">
                        <?php echo e($patient['initials'] ?? strtoupper(substr($patient->name ?? '', 0, 2))); ?>

                    </div>
                    <?php if(($patient['unread'] ?? 0) > 0): ?>
                    <span class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
                        <?php echo e($patient['unread'] ?? $patient->unread_count ?? 0); ?>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <h3 class="font-display font-bold text-sm text-gray-900 truncate group-hover:text-blue-700 transition-colors">
                                <?php echo e($patient['name'] ?? $patient->name); ?>

                            </h3>
                            <p class="text-xs text-gray-400 mt-0.5">
                                <?php echo e($patient['age'] ?? $patient->age); ?>y ·
                                <?php echo e($patient['gender'] ?? $patient->gender); ?> ·
                                <?php echo e($patient['blood'] ?? $patient->blood_group); ?>

                            </p>
                        </div>
                        <?php if($patient['abha'] ?? $patient->abha_id): ?>
                    ...
                    <div class="flex items-center gap-1.5 mt-2">
                        ...
                        <span class="text-xs text-gray-500"><?php echo e($patient['last_visit'] ?? $patient->last_visit); ?></span>
                        <span class="text-xs text-gray-300">·</span>
                        <span class="text-xs font-medium" style="color:#0891B2;"><?php echo e($patient['visit_type'] ?? $patient->last_visit_type); ?></span>
                    </div>

                    
                    <?php $conditions = $patient['conditions'] ?? ($patient->conditions ?? []); ?>
                    <?php if(count($conditions) > 0): ?>
                    <div class="flex flex-wrap gap-1 mt-2">
                        <?php $__currentLoopData = array_slice($conditions, 0, 2); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cond): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-600"><?php echo e($cond); ?></span>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php if(count($conditions) > 2): ?>
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-400">+<?php echo e(count($conditions)-2); ?></span>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
    ...
    <?php if(isset($patients) && method_exists($patients, 'links')): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/patients/index.blade.php ENDPATH**/ ?>