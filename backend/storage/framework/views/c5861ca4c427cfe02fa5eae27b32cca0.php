<?php $__env->startSection('title', 'Pharmacy Inventory'); ?>
<?php $__env->startSection('breadcrumb', 'Pharmacy Inventory'); ?>
...
<div id="pharmacy-inventory-root" x-data="inventoryPage()" class="p-4 sm:p-5 lg:p-7 space-y-5">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex gap-2">
            ...
            <a href="<?php echo e(url('/pharmacy')); ?>" class="px-3 py-2 text-sm font-semibold text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                Back
            </a>
            <a href="<?php echo e(url('/pharmacy/items/create')); ?>" class="px-3 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50">
                Full form
            </a>
    ...
    <div class="flex flex-wrap gap-2">
        <a href="<?php echo e(url('/pharmacy/inventory?view=clinic')); ?>"
            class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold border transition-colors <?php echo e($isNational ? 'border-gray-200 text-gray-600 bg-white hover:bg-gray-50' : 'border-blue-200 text-white shadow-sm'); ?>"
            style="<?php echo e($isNational ? '' : 'background:linear-gradient(135deg,#1447E6,#0891B2);'); ?>">
            My clinic inventory
        </a>
        <a href="<?php echo e(url('/pharmacy/inventory?view=national')); ?>"
            class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold border transition-colors <?php echo e($isNational ? 'border-blue-200 text-white shadow-sm' : 'border-gray-200 text-gray-600 bg-white hover:bg-gray-50'); ?>"
            style="<?php echo e($isNational ? 'background:linear-gradient(135deg,#1447E6,#0891B2);' : ''); ?>">
            National database
        </a>
        <span class="text-xs text-gray-500 self-center hidden sm:inline">~<?php echo e(number_format((float) ($catalogCount ?? 0), 0)); ?> products in imported catalog</span>
    </div>
    ...
    <?php echo $__env->make('pharmacy.partials.catalog-stock-hint', ['context' => 'inventory'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

    
    <div class="bg-white border border-gray-200 rounded-xl p-4">
        <form method="GET" action="<?php echo e(url('/pharmacy/inventory')); ?>" class="flex flex-col sm:flex-row gap-3">
            <?php if($isNational): ?>
            ...
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="<?php echo e($isNational ? 'Search product name, manufacturer, composition…' : 'Search medicine name, generic name…'); ?>"
            ...
            <select name="category" class="px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <?php $__currentLoopData = $categories ?? ['Antibiotics', 'Analgesics', 'Antacids', 'Vitamins', 'Cardiac', 'Diabetic', 'Others']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <select name="stock_status" class="px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="in-stock"    <?php echo e(request('stock_status') === 'in-stock'    ? 'selected' : ''); ?>>In Stock</option>
                <option value="low-stock"   <?php echo e(request('stock_status') === 'low-stock'   ? 'selected' : ''); ?>>Low Stock</option>
            ...
            <button type="submit" class="px-4 py-2 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-700 transition-colors">
                <?php echo e($isNational ? 'Search' : 'Filter'); ?>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                <h3 class="text-sm font-bold text-gray-900"><?php echo e($isNational ? 'National medicine database' : 'My clinic stock'); ?></h3>
                <?php if($isNational): ?>
            ...
            <span class="text-xs text-gray-400"><?php echo e((int) ($items->total() ?? 0)); ?> items</span>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <p class="text-sm font-semibold text-gray-900"><?php echo e($row->name ?? '—'); ?></p>
                            <p class="text-xs text-gray-400">ID <?php echo e($row->id); ?></p>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-600 max-w-[14rem] break-words"><?php echo e($row->manufacturer ?: '—'); ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 max-w-md">
                            <?php echo e(\Illuminate\Support\Str::limit((string) ($row->composition ?? ''), 120)); ?>

                        </td>
                        <td class="px-4 py-3 text-sm font-semibold text-gray-900">
                            <?php if($row->mrp !== null): ?>
                                ₹<?php echo e(number_format((float) $row->mrp, 2)); ?>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600">
                            <?php if($row->rx_required === true || ($row->prescription_label ?? '') !== ''): ?>
                        ...
                        <td class="px-4 py-3 relative z-[60] align-middle">
                            ...
                            <button type="button"
                                ...
                                data-pharmacy-catalog="<?php echo e(e($pharmacyCatalogJson)); ?>"
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            ...
                            <?php if($item->generic_name ?? null): ?>
                            <p class="text-xs text-gray-400"><?php echo e($item->generic_name); ?></p>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($item->category ?? '—'); ?></td>
                        <td class="px-4 py-3">
                            <span class="text-sm font-semibold text-gray-900"><?php echo e($stockQty); ?></span>
                            <span class="text-xs text-gray-400 ml-1"><?php echo e($item->unit ?? ''); ?></span>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($reorderLvl); ?> <?php echo e($item->unit ?? ''); ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600">
                            <?php if($item->nearest_expiry ?? null): ?>
                                <?php
                                    $expiry = \Carbon\Carbon::parse($item->nearest_expiry);
                                    $expiring = $expiry->diffInDays(now()) <= 90 && $expiry->isFuture();
                                ?>
                                <span class="<?php echo e($expiring ? 'text-amber-600 font-semibold' : ''); ?>">
                                    <?php echo e($expiry->format('M Y')); ?>

                                    <?php if($expiring): ?> (soon)<?php endif; ?>
                                </span>
                            <?php else: ?>
                                <span class="text-gray-400">—</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-3 text-sm font-semibold text-gray-900">₹<?php echo e(number_format((float) ($item->price_per_unit ?? 0), 2)); ?></td>
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold"
                                  style="background:<?php echo e($stockStatus['bg']); ?>;color:<?php echo e($stockStatus['color']); ?>;">
                                <?php echo e($stockStatus['label']); ?>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1">
                                ...
                                <button type="button" class="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Add Stock"
                                    data-item-name="<?php echo e(e($item->name ?? '')); ?>"
                                    @click="openAddStock(<?php echo e($item->id); ?>, $event.currentTarget.getAttribute('data-item-name') || '')">
    ...
    <div x-show="showAddModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <form id="pharmacy-add-item-form" method="POST" action="<?php echo e(url('/pharmacy/items')); ?>" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="medicine_catalog_id" value="">
                    <p x-show="!addFromCatalog" class="text-xs text-gray-500 sm:col-span-2">Type in <strong>Medicine name</strong> to search the national catalog (~<?php echo e(number_format((float) ($catalogCount ?? 0), 0)); ?> products). Click a row to auto-fill.</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="category" required class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                                ...
                                <?php $__currentLoopData = ['Antibiotics', 'Analgesics', 'Antacids', 'Vitamins', 'Cardiac', 'Diabetic', 'Dermatology', 'Others']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div x-show="showStockModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full">
                ...
                <form method="POST" action="<?php echo e(url('/pharmacy/stock-in')); ?>" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
...
function inventoryPage() {
    return {
        ...
        catalogSearchUrl: <?php echo json_encode($inventoryMedicineCatalogSearchUrl, 15, 512) ?>,
        catalogResults: [],
        catalogLoading: false,
        catalogSearchTimer: null,
        viewMode: <?php echo json_encode($viewMode ?? 'clinic', 15, 512) ?>,
        init() {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/pharmacy/inventory.blade.php ENDPATH**/ ?>