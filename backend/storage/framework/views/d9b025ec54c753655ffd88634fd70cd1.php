<?php $__env->startSection('title', 'Photo Vault'); ?>
<?php $__env->startSection('breadcrumb', 'Photo Vault'); ?>
...
<div class="p-6 space-y-6">
        ...
        <form method="GET" class="flex flex-wrap items-center gap-4">
            <div>
                ...
                <select name="patient_id" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-[180px]">
                    ...
                    <?php $__currentLoopData = $patientsWithPhotos ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $patient): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div>
                ...
                <select name="type" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="before" <?php echo e(request('type') === 'before' ? 'selected' : ''); ?>>Before</option>
                    <option value="after" <?php echo e(request('type') === 'after' ? 'selected' : ''); ?>>After</option>
                    <option value="progress" <?php echo e(request('type') === 'progress' ? 'selected' : ''); ?>>Progress</option>
                    <option value="clinical" <?php echo e(request('type') === 'clinical' ? 'selected' : ''); ?>>Clinical</option>
                </select>
            </div>
            <div>
                ...
                <select name="region" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <?php $__currentLoopData = $bodyRegions ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $region): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($region); ?>" <?php echo e(request('region') === $region ? 'selected' : ''); ?>>
                        <?php echo e(ucfirst($region)); ?>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="<?php echo e(route('photo-vault.index')); ?>" class="px-4 py-2 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition-colors">
                    Clear
                </a>
            </div>
        </form>
    ...
    <?php if(isset($photos) && (is_object($photos) ? $photos->count() : count($photos)) > 0): ?>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                ...
                <div class="group relative bg-gray-100 rounded-xl overflow-hidden aspect-square" x-data="{ showDelete: false }">
                    ...
                    <div class="absolute top-2 left-2 z-10">
                        <span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold text-white <?php echo e($typeColors[$photo->photo_type] ?? 'bg-gray-500'); ?>">
                    ...
                    <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        <p class="text-white text-xs font-medium truncate"><?php echo e($photo->patient?->name ?? 'Unknown'); ?></p>
                        <p class="text-white/70 text-[10px] truncate"><?php echo e($photo->body_region ?? 'No region'); ?></p>
                        <p class="text-white/50 text-[10px]"><?php echo e($photo->created_at?->format('d M Y')); ?></p>
                    </div>
                    ...
                    <div x-show="showDelete" 
                        ...
                        <div class="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full" @click.stop>
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-500 mt-2">This will permanently delete the photo for <strong><?php echo e($photo->patient?->name ?? 'Unknown'); ?></strong>.</p>
                            </div>
                            <div class="flex gap-3 mt-6">
                                ...
                                <form action="<?php echo e(route('patients.delete-photo', ['patient' => $photo->patient_id, 'photo' => $photo->id])); ?>" 
        ...
        <?php if(method_exists($photos, 'links')): ?>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
        ...
        <div class="mt-6">
            <a href="<?php echo e(route('patients.index')); ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <?php if(isset($recentByPatient) && $recentByPatient->count() > 0): ?>
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200">
            <h3 class="font-semibold text-gray-900">Recent Uploads by Patient</h3>
        </div>
        <div class="divide-y divide-gray-100">
            <?php $__currentLoopData = $recentByPatient; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if($item->patient): ?>
            <a href="<?php echo e(route('photo-vault.index', ['patient_id' => $item->patient_id])); ?>" class="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-white font-bold">
                    <?php echo e(strtoupper(substr($item->patient->name ?? 'P', 0, 1))); ?>

                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900"><?php echo e($item->patient->name); ?></p>
                    <p class="text-xs text-gray-500"><?php echo e($item->photo_count); ?> photos</p>
                </div>
                <div class="text-right">
                    <p class="text-xs text-gray-400">Last upload</p>
                    <p class="text-sm text-gray-600"><?php echo e(\Carbon\Carbon::parse($item->latest)->diffForHumans()); ?></p>
                </div>
            </a>
            <?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/photo-vault/index.blade.php ENDPATH**/ ?>