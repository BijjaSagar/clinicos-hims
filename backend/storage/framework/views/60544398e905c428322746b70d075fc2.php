<?php $__env->startSection('title', 'Clinic Locations'); ?>
...
<div x-data="locationsManager()" class="p-6 space-y-6">
    <?php if(isset($locationsSchemaReady) && !$locationsSchemaReady): ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl p-5 border border-gray-200">
            <div class="text-3xl font-bold text-blue-600"><?php echo e(count($locations)); ?></div>
            <div class="text-gray-600">Total Locations</div>
        </div>
        <div class="bg-white rounded-xl p-5 border border-gray-200">
            <div class="text-3xl font-bold text-green-600"><?php echo e(collect($locationStats)->sum(fn($s) => $s['rooms'])); ?></div>
            <div class="text-gray-600">Total Rooms</div>
        </div>
        <div class="bg-white rounded-xl p-5 border border-gray-200">
            <div class="text-3xl font-bold text-purple-600"><?php echo e(collect($locationStats)->sum(fn($s) => $s['doctors'])); ?></div>
            <div class="text-gray-600">Assigned Doctors</div>
        </div>
        <div class="bg-white rounded-xl p-5 border border-gray-200">
            <div class="text-3xl font-bold text-orange-600"><?php echo e(collect($locationStats)->sum(fn($s) => $s['appointments_today'])); ?></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="location-card <?php echo e($location->is_primary ? 'primary' : ''); ?>">
            <div class="flex items-start justify-between mb-4">
                <div>
                    <div class="flex items-center gap-2">
                        <h3 class="text-lg font-semibold text-gray-900"><?php echo e($location->name); ?></h3>
                    ...
                    <p class="text-gray-600 text-sm mt-1"><?php echo e($location->address); ?></p>
                </div>
                <div class="flex items-center gap-2">
                    <button @click="editLocation(<?php echo e(json_encode($location)); ?>)" class="p-2 hover:bg-gray-100 rounded-lg text-gray-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                        </svg>
                    </button>
                    <?php if(!$location->is_primary): ?>
                    <button @click="deleteLocation(<?php echo e($location->id); ?>)" class="p-2 hover:bg-red-50 rounded-lg text-red-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex flex-wrap gap-3 mb-4">
                <span class="stat-badge">
                    ...
                    <?php echo e($locationStats[$location->id]['rooms'] ?? 0); ?> Rooms
                </span>
                <span class="stat-badge">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                    </svg>
                    <?php echo e($locationStats[$location->id]['doctors'] ?? 0); ?> Doctors
                </span>
                <span class="stat-badge">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                    </svg>
                    <?php echo e($locationStats[$location->id]['appointments_today'] ?? 0); ?> Today
                </span>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/akash/Downloads/clinicos-main/backend/resources/views/settings/locations.blade.php ENDPATH**/ ?>