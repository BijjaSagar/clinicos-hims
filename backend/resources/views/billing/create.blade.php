...
<div class="p-6">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-4 p-3 bg-indigo-50 border border-indigo-200 rounded-lg text-sm text-indigo-900">
            <strong>IPD billing:</strong> This invoice will link to admission #{{ request('admission_id') }}. Edit line amounts as needed (ward, procedures, pharmacy, lab).
        </div>
        @elseif(request('visit_id'))
        <div class="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-900">
            <strong>OPD billing:</strong> Linked to <strong>Visit #{{ request('visit_id') }}</strong> — totals will show under this encounter in EMR.
        </div>
        @else
        ...
        <form action="{{ route('billing.store') }}" method="POST" x-data="invoiceForm({{ \Illuminate\Support\Js::from($invoiceFormAlpineDefaults) }})" x-init="calculateTotals()" class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-xl border border-gray-200">
                ...
                <div class="p-5">
                    <select name="patient_id" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="{{ $patient->id }}" {{ (string) request('patient_id') === (string) $patient->id ? 'selected' : '' }}>
            ...
            <div class="bg-white rounded-xl border border-gray-200">
                ...
                <div class="p-5">
                    {{-- Div/grid instead of <table>/<tbody>/<template x-for> — Alpine reliably mutates rows here. --}}
...
<script>
(function () {
    var ctx = @json($billingCreatePageContext);
