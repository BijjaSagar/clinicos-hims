...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $activeCount ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            <p class="text-xs font-semibold uppercase tracking-wider text-brand-blue mb-1">Categories</p>
            <p class="text-3xl font-bold text-gray-900">{{ $categoryCount ?? ($categories ?? collect())->count() }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-4">
                            <p class="font-medium text-gray-900">{{ $test->name ?? $test->test_name ?? '—' }}</p>
                            @if($test->reference_range ?? null)
                                <p class="text-xs text-gray-400 mt-0.5 truncate max-w-xs">Ref: {{ $test->reference_range }}</p>
                            @endif
                        </td>
                        <td class="px-4 py-4">
                            <span class="font-mono text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{{ $test->code ?? $test->test_code ?? '—' }}</span>
                        </td>
                        <td class="px-4 py-4 text-gray-600">{{ $test->category ?? $test->department_name ?? '—' }}</td>
                        ...
                        <td class="px-4 py-4 text-center text-gray-600">{{ $test->turnaround_hours ?? $test->tat_hours ?? '—' }}</td>
