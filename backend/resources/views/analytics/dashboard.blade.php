...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
    ...
    @include('analytics.partials.subnav', ['active' => 'index'])
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div class="relative overflow-hidden rounded-2xl shadow-sm text-white p-5 bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-800 ring-1 ring-white/10">
            ...
            <p class="mt-3 text-3xl font-extrabold font-display tabular-nums">{{ number_format($data['patients']['total']) }}</p>
            <p class="mt-1 text-xs text-white/75">+{{ $data['patients']['new_this_month'] }} this month</p>
        </div>
        ...
        <div class="relative overflow-hidden rounded-2xl shadow-sm text-white p-5 bg-gradient-to-br from-fuchsia-500 via-pink-500 to-rose-600 ring-1 ring-white/10">
            ...
            <p class="mt-3 text-3xl font-extrabold font-display tabular-nums">{{ $data['appointments']['today'] }}</p>
            <p class="mt-1 text-xs text-white/75">{{ $data['appointments']['this_month'] }} this month</p>
        </div>
        ...
        <div class="relative overflow-hidden rounded-2xl shadow-sm text-white p-5 bg-gradient-to-br from-sky-500 via-cyan-500 to-teal-600 ring-1 ring-white/10">
            ...
            <p class="mt-3 text-3xl font-extrabold font-display tabular-nums">₹{{ number_format($data['revenue']['total_today']) }}</p>
            <p class="mt-1 text-xs text-white/75">₹{{ number_format($data['revenue']['total_month']) }} this month</p>
        </div>
        ...
        <div class="relative overflow-hidden rounded-2xl shadow-sm text-white p-5 bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-600 ring-1 ring-white/10">
            ...
            <p class="mt-3 text-3xl font-extrabold font-display tabular-nums">{{ count($data['top_diagnoses']) }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        @foreach($dailyRev as $day)
                            ...
                            <div class="flex-1 min-w-0 flex flex-col justify-end group">
                                <div
                                    ...
                                    title="{{ $d }}: ₹{{ number_format($dayTotal) }}"
                                ></div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="lg:col-span-4 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col min-h-[280px]">
            ...
            <div class="divide-y divide-gray-100 flex-1 overflow-y-auto max-h-[320px]">
                @forelse($data['top_diagnoses'] as $dx)
                    <div class="flex items-center justify-between gap-3 px-5 py-3 hover:bg-gray-50/80 transition-colors">
                        <span class="text-sm text-gray-800 truncate" title="{{ data_get($dx, 'diagnosis_text') }}">{{ data_get($dx, 'diagnosis_text') }}</span>
                        <span class="shrink-0 inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-bold bg-brand-blue/10 text-brand-blue tabular-nums">{{ data_get($dx, 'count') }}</span>
                    </div>
                @empty
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($data['doctor_performance'] as $doc)
                            <tr class="hover:bg-gray-50/60">
                                <td class="px-5 py-3 text-gray-800">{{ data_get($doc, 'doctor') }}</td>
                                <td class="px-5 py-3 text-right font-semibold text-gray-900 tabular-nums">{{ data_get($doc, 'patients_seen') }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div class="p-5 space-y-4">
                @if(count($data['patients']['gender_dist']) > 0)
                    @foreach($data['patients']['gender_dist'] as $gender => $count)
                        @php
                        ...
                        <div>
                            <div class="flex justify-between text-sm mb-1.5">
                                <span class="font-medium text-gray-700">{{ ucfirst($gender ?? 'N/A') }}</span>
