...
<div x-data="{ statusFilter: '{{ request('status', 'all') }}', search: '{{ request('search', '') }}' }"
    ...
    <div class="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-gray-50/80 px-4 py-3 text-sm">
        ...
        @if(\Illuminate\Support\Facades\Route::has('opd.queue'))
            <a href="{{ route('opd.queue') }}" class="px-3 py-1.5 rounded-lg font-medium text-indigo-700 bg-white border border-indigo-100 hover:bg-indigo-50">OPD Queue</a>
        @endif
        @if(\Illuminate\Support\Facades\Route::has('laboratory.index'))
            <a href="{{ route('laboratory.index') }}" class="px-3 py-1.5 rounded-lg font-medium text-teal-800 bg-white border border-teal-100 hover:bg-teal-50">Laboratory</a>
        @endif
        @if(\Illuminate\Support\Facades\Route::has('pharmacy.index'))
            <a href="{{ route('pharmacy.index') }}" class="px-3 py-1.5 rounded-lg font-medium text-emerald-800 bg-white border border-emerald-100 hover:bg-emerald-50">Pharmacy</a>
        @endif
        @if(\Illuminate\Support\Facades\Route::has('billing.index'))
            <a href="{{ route('billing.index') }}" class="px-3 py-1.5 rounded-lg font-medium text-blue-800 bg-white border border-blue-100 hover:bg-blue-50">Billing</a>
        @endif
    </div>
