    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2 text-xs font-semibold">
            @if(\Illuminate\Support\Facades\Route::has('opd.queue'))
                <a href="{{ route('opd.queue') }}" class="px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-800 border border-indigo-100 hover:bg-indigo-100">OPD Queue</a>
            @endif
            @if(\Illuminate\Support\Facades\Route::has('ipd.index'))
                <a href="{{ route('ipd.index') }}" class="px-3 py-1.5 rounded-lg bg-slate-50 text-slate-800 border border-slate-200 hover:bg-slate-100">IPD</a>
            @endif
            @if(\Illuminate\Support\Facades\Route::has('laboratory.index'))
                <a href="{{ route('laboratory.index') }}" class="px-3 py-1.5 rounded-lg bg-teal-50 text-teal-900 border border-teal-100 hover:bg-teal-100">Laboratory</a>
            @endif
            @if(\Illuminate\Support\Facades\Route::has('billing.index'))
                <a href="{{ route('billing.index') }}" class="px-3 py-1.5 rounded-lg bg-blue-50 text-blue-900 border border-blue-100 hover:bg-blue-100">Billing</a>
            @endif
        </div>
    </div>
