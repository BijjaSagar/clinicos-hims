...
<div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="bg-white rounded-xl border p-4">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div>
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Rx number or patient..."
                       class="w-full text-sm rounded-lg border-gray-300 focus:ring-teal-500 focus:border-teal-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">From</label>
                <input type="date" name="from" value="{{ request('from') }}" class="w-full text-sm rounded-lg border-gray-300 focus:ring-teal-500 focus:border-teal-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">To</label>
                <input type="date" name="to" value="{{ request('to') }}" class="w-full text-sm rounded-lg border-gray-300 focus:ring-teal-500 focus:border-teal-500">
            </div>
    ...
    <div class="bg-white rounded-xl border shadow-sm">
        ...
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($dispensings as $d)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $d->dispensing_number ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $d->patient?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-500">{{ $d->items->count() }} item(s)</td>
                            <td class="px-4 py-3 text-sm font-semibold text-gray-900">₹{{ number_format($d->total, 2) }}</td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-700">{{ ucfirst($d->payment_mode ?? '—') }}</span>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-500">{{ $d->dispensedBy?->name ?? '—' }}</td>
            ...
            <div class="px-6 py-4 border-t">{{ $dispensings->links() }}</div>
        @endif
    </div>
</div>
@endsection
